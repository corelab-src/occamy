//===----------------------------------------------------------------------===//
// DNN to LLVM: DNNMemPoolInitOpLowering
//===----------------------------------------------------------------------===//
...
class DNNMemPoolInitOpLowering : public ConvertToLLVMPattern {
public:
  DNNMemPoolInitOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::DNNMemPoolInitOp::getOperationName(), ctx, typeConverter) {}
  ...
  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    auto callMemPoolInit = core_dnn::DNNRuntimeAPI::callApi(rewriter, loc,
        apiRegistry, DNNRuntimeAPI::API::CUDA_MEMPOOLINIT,
...
void mlir::populateDNNMemPoolInitToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter) {
  patterns.insert<DNNMemPoolInitOpLowering>(ctx, typeConverter);
}
