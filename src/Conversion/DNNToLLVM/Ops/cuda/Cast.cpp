...
class DNNCastOpLowering : public ConvertToLLVMPattern {
public:
  DNNCastOpLowering(MLIRContext *ctx, LLVMTypeConverter &typeConverter)
      : ConvertToLLVMPattern(
          mlir::DNNCastOp::getOperationName(), ctx, typeConverter) {}

  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
      ConversionPatternRewriter &rewriter) const final {

    DNNCastOp castOp = dyn_cast<DNNCastOp>(op);
    ...
    auto callCast = DNNRuntimeAPI::callApi(rewriter, loc,
        apiRegistry, DNNRuntimeAPI::API::CUDA_CAST,
...
void mlir::populateDNNCastToLLVMConversionPattern(
    RewritePatternSet &patterns, MLIRContext *ctx, LLVMTypeConverter &typeConverter) {
  patterns.insert<DNNCastOpLowering>(ctx, typeConverter);
}
